...
@FXML private HBox      categoriasBar;
    ...
    /**
     * ID del usuario que abrió ESTE dashboard. Se captura una única vez en
     * initialize() desde UserStore y ya no se vuelve a leer el global mutable.
     * Esto elimina el TOCTOU: si UserStore.usuarioActivo cambia después (logout
     * de otra sesión, bug de navegación, etc.) este controlador sigue operando
     * sobre el usuario correcto y nunca graba pedidos con el ID equivocado.
     */
    ...
    private void cargarDatosUsuario() {
        ...
        // Snapshot inmutable: a partir de aquí todo el controlador usa currentUserId
        // en lugar de volver a consultar el global mutable UserStore.getUsuarioActivo().
        // Esto rompe el TOCTOU que permitía que un cambio externo de sesión causara
        // que pedidos se grabaran con idUsuario incorrecto.
    ...
    @FXML private void navegarPedidos()  {
        PedidoStore.getGestor().recargarDatos(); // asegura que cada usuario ve solo sus propios pedidos
        actualizarVistaPedidos();
        mostrarSeccion(panePedidos, navBtnPedidos);
    }
